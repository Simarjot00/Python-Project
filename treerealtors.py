...
def realtr():
     ...
     def tree():
               ...
               cur=info(query)
               ws=Tk()
               ws.title('Realtors Data')
               ws.geometry('800x600+100+100')
               ws['bg']='white'
               columns=('realtor_id','name','phone','licenseno','email')
               treeview_frame = TreeviewFrame()
               treeview_frame.pack()
               treeview_frame.treeview.config(columns=columns, show="headings")
               treeview_frame.treeview.heading('realtor_id',text='ID',anchor=CENTER)
               ...
               treeview_frame.treeview.heading('licenseno',text='lno',anchor=CENTER)
